...
class NetworkController {
    ...
    func postDeadline(token: Token?,
                      representation: DeadlineRepresentation,
                      studentID: Int,
        ...
        do {
            ...
            NSLog("Error - Error encoding deadline representation " +
                String(describing: error) +
                " " +
                String(describing: error.localizedDescription))
        }
    ...
    func postNotification(token: Token?,
        ...
        do {
            ...
            NSLog("Error - Error encoding notification representation " +
                String(describing: error) +
                " " +
                String(describing: error.localizedDescription))
        }
        URLSession.shared.dataTask(with: request) { _, response, error in
            if let error = error {
                NSLog("Error - Error posting notification: " +
                    String(describing: error) +
                    " " +
                    String(describing: error.localizedDescription))
                return completion(.failure(.otherError))
    ...
    func deleteStudent(token: Token?, studentID: Int, completion: @escaping CompletionHandler) {
        ...
        URLSession.shared.dataTask(with: request) { _, response, error in
            if let error = error {
                NSLog("Error - Error deleting student from remote host: " +
                    String(describing: error) +
                    " " +
                    String(describing: error.localizedDescription))
                return completion(.failure(.otherError))
            ...
            guard let response = response as? HTTPURLResponse,
                response.statusCode == 200 else {
                    NSLog("Error - Bad response when deleting student from remote host: " +
                        String(describing: error) +
                        " " +
                        String(describing: error?.localizedDescription))
                    return completion(.failure(.badResponse))
    ...
    func deleteDeadline(token: Token?, studentID: Int, deadlineID: Int, completion: @escaping CompletionHandler) {
        ...
        URLSession.shared.dataTask(with: request) { _, response, error in
            if let error = error {
                NSLog("Error - Error deleting deadline from remote host: " +
                    String(describing: error) +
                    " " +
                    String(describing: error.localizedDescription))
                return completion(.failure(.otherError))
            ...
            guard let response = response as? HTTPURLResponse,
                response.statusCode == 200 else {
                    NSLog("Error - Bad response when deleting deadline from remote host: " +
                        String(describing: error) +
                        " " +
                        String(describing: error?.localizedDescription))
                    return completion(.failure(.badResponse))
    ...
    func deleteNotification(token: Token?,
                            studentID: Int,
                            deadlineID: Int,
                            notificationID: Int,
                            completion: @escaping CompletionHandler) {
        ...
        URLSession.shared.dataTask(with: request) { _, response, error in
            if let error = error {
                NSLog("Error - Error deleting notification from remote host: " +
                    String(describing: error) +
                    " " +
                    String(describing: error.localizedDescription))
                return completion(.failure(.otherError))
            ...
            guard let response = response as? HTTPURLResponse,
                response.statusCode == 200 else {
                    NSLog("Error - Bad response when deleting notification from remote host: " +
                        String(describing: error) +
                        " " +
                        String(describing: error?.localizedDescription))
                    return completion(.failure(.badResponse))
